...
To resolve a public law: `collection:PLAW lawtype:public congress:{n} docnumber:{m}`. Corrected 2026-09-20 (O61): this line had pinned the query without `lawtype` and said the term "joins the query only if a collision is ever observed; none has been" — nobody had looked. `docnumber` is shared between the two series, so the bare query returns the public and the private law together for every private law that exists, 60 of 60, the private one first in 34; with `lawtype:public` it returns exactly `PLAW-{n}publ{m}`, 60 of 60 (O61a). O12's one pair, 118/31, has no private twin. Measured 2026-09-15: `lawtype` partitions PLAW exactly (private 60 + public 5,939 = 5,999) and composes with `publishdate` ranges and `uscodecitation` without changing public-law counts (O44e) — so it is safe to add wherever private packages must be excluded, as the R14 detector does.
