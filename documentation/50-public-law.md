...
The composition this enables: resolve a section as codified (`get_us_code_section`), note its `currentthrough` date, then `search_public_laws` with `uscodecitation` for later laws touching it. The server documents this recipe; it does not automate the join, and the reason is now measured, not hypothetical: the `uscodecitation` search field misses ~a quarter of sampled (law, section) pairs that the laws' own package metadata lists — 25/33 recall on the E9 sample, structural rather than recency-driven (O21, O17/O18). Automating over that field would launder its gaps as completeness. The package-summary `references` array is the designated higher-recall v2 source (Q6, E9).
