...
An explicit non-goal, stated here so it cannot be mistaken for an oversight: this is not the most current codification available. That is OLRC's release points at uscode.house.gov, and they are declined for three reasons that compound (R1). Retrieval mechanics are title-level — per-title and all-titles ZIPs (S5 quoting the download page; the maintainer's own note in Q11) — so fetching one current section means ingesting a title, which is the local corpus R1 rules out. The markup is USLM, a different vocabulary from the `field-start`/`field-end` markers that the `structure` block is derived from (O36), so a validated feature (R12b/R13, O42a) would have to be re-derived on a second format with no measurement that it survives the move. And a release point is still behind current enacted law — S5 puts it at roughly six weeks, unmeasured here — so switching would shrink the currency gap, not close it. The gap between any codification and enacted law is what PLAW exists to fill, and it fills it the same way whichever codification sits under it. The posture is therefore a stale-but-labelled Code plus a live PLAW layer, with the staleness surfaced per lookup (R14), not a chase after currency.
